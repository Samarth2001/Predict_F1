...
def run_prediction_workflow(force_fetch=False):
    ...
    if not os.path.exists(config.RACES_CSV_PATH) or not os.path.exists(
        config.QUALI_CSV_PATH
    ):
    ...
    if loaded_data is None:
        ...
        return
    hist_races, hist_quali, curr_races, curr_quali, upcoming_info, latest_quali = (
        loaded_data
    )
    ...
    target_column = 'Quali_Pos'

    # Extract features and target
    X_quali = features_df.drop(columns=[target_column])
    y_quali = features_df[target_column]

    # Train qualifying model using legacy mode for compatibility
